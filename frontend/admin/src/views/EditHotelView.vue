<template>
  <main class="page-container">
    ...
    <div v-if="isLoadingData" class="loading-state">Loading hotel data...</div>

    <form v-else @submit.prevent="handleUpdate" class="hotel-form">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input v-model="form.shortDescription" type="text" :class="{ 'input-error': errors.shortDescription }"
            required />
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input v-model="form.phoneNumber" type="text" :class="{ 'input-error': errors.phoneNumber }" required />
          <span v-if="errors.phoneNumber" class="error-text">{{ errors.phoneNumber }}</span>
        </div>
        <div class="form-group">
          <label>Email</label>
          <input v-model="form.email" type="email" :class="{ 'input-error': errors.email }" required />
...
onMounted(async () => {
  await hotelStore.fetchAmenitiesByCategory('hotel');
  const data = await hotelStore.getHotelForEdit(hotelId);
  ...
  if (data) {
    ...
    toast.error('Load Failed', 'Failed to load hotel data.');
...
const handleUpdate = async () => {
  ...
  const result = await hotelStore.updateHotel(hotelId, form.value);
